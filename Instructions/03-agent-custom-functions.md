...
1. Digite o seguinte comando para alterar o diretório de trabalho para a pasta que contém os arquivos de código e liste todos eles.
...
1. Localize o comentário **Create a function to submit a support ticket** e adicione o código a seguir, que gera um número de tíquete e salva um tíquete de suporte como um arquivo de texto.
...
1. Localize o comentário **Define a set of callable functions** e adicione o código a seguir, que define estaticamente um conjunto de funções que podem ser chamadas neste arquivo de código (nesse caso, há apenas uma, mas em uma solução real, o agente pode chamar várias funções):
...
### Escreva código para implementar um agente que possa usar a sua função

1. Digite o comando a seguir para começar a editiar o código de agente.

    ```
    code agent.py
    ```

    > **Dica**: ao adicionar código ao arquivo de código, certifique-se de manter o recuo correto.
...
1. Localize o comentário **Add references** e adicione o seguinte código para importar as classes necessárias para criar um agente de IA do Azure que usa o seu código de função como uma ferramenta:
...
1. Encontre o comentário **Connect to the Azure AI Foundry project** e adicione o seguinte código para se conectar ao projeto da IA do Azure usando as credenciais atuais do Azure.
...
1. Localize o comentário **Define an agent that can use the custom functions** e adicione o código a seguir para adicionar o seu código de função a um conjunto de ferramentas e, em seguida, crie um agente que possa usar o conjunto de ferramentas e um thread no qual executar a sessão de chat.
...
1. Localize o comentário **Send a prompt to the agent** e adicione o código a seguir para adicionar o prompt do usuário como uma mensagem e executar o thread.
   ...
   run = project_client.agents.create_and_process_run(thread_id=thread.id, agent_id=agent.id)
    ...
    > **Observação**: usar o método **create_and_process_run** para executar o thread permite que o agente encontre automaticamente suas funções e opte por usá-las com base em seus nomes e parâmetros. Como alternativa, você pode usar o método **create_run**, caso em que você seria responsável por escrever código para sondar o status de execução para determinar quando uma chamada de função é necessária, chamar a função e retornar os resultados ao agente.

1. Localize o comentário **Check the run status for failures** no status de execução e adicione o código a seguir para mostrar os erros que ocorrerem.
...
1. Localize o comentário **Show the latest response from the agent** e adicione o código a seguir para recuperar as mensagens do thread concluído e exibir a última que foi enviada pelo agente.
...
1. Localize o comentário **Get the conversation history**, que ocorre após o término do loop, e adicione o seguinte código para imprimir as mensagens do tópico de conversa; invertendo a ordem para mostrá-las em sequência cronológica
...
1. Localize o comentário **Clean up** e adicione o código a seguir para excluir o agente e o thread quando não forem mais necessários.
...
1. Revise o código, usando os comentários para entender como:
    ...
    - Executa um thread com uma mensagem de prompt do usuário.
    - Verifica o status da execução caso haja uma falha
    - Recupera as mensagens do thread completado e exibe a última enviada pelo agente.
    - Exibe o histórico da conversa
    - Exclui o agente e o thread quando eles não são mais necessários.
...
1. Exiba a resposta. O agente pode solicitar seu endereço de email e uma descrição do problema. Você pode usar qualquer endereço de email (por exemplo, `alex@contoso.com`) e qualquer descrição de problema (por exemplo, `my computer won't start`)

    Quando tiver informações suficientes, o agente escolherá usar a sua função conforme necessário.

1. Você pode continuar a conversa, se quiser. O thread é *stateful*, portanto, ele retém o histórico de conversas, o que significa que o agente tem o contexto completo para cada resposta. Quando terminar, digite `quit`.
1. Revise as mensagens de conversa que foram recuperadas da conversa e os tickets que foram gerados.
1. A ferramenta terá salvado tíquetes de suporte na pasta do aplicativo. Você pode usar o comando `ls` para verificar e, em seguida, usar o comando `cat` para exibir o conteúdo do arquivo, assim:
